...
$sql = "SELECT * FROM `n_seolink_url` WHERE `ID` NOT IN (SELECT `URL` AS `ID` FROM `n_seolink_stats` WHERE `URL`>0 GROUP BY `URL`) ";
if ($by != '' && $order != '') {
...
foreach ($arList as $n => $arRow) {
	...
	$row->AddViewField("NAME", '<a href="nasledie.seolink_url_edit.php?ID=' . $arRow['ID'] . '&lang=' . LANG . '">' . $arRow['NAME'] . '</a>');
	$row->AddViewField("URL", $arRow['URL']);
}
